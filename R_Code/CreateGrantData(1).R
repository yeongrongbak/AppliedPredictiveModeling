...
raw <- read.csv("C:/Users/student/Desktop/I2_APM_R/all/unimelb_training.csv")
...
# vertical <- subset(vertical, Role != "") 아래의 방법을 사용

ind <- which(vertical$Role == "" | is.na(vertical$Role))
vertical <- vertical[-ind,]
...
vertical$Home.Language <- gsub("Other", "OtherLang", as.character(vertical$Home.Language)) # Other는 OtherLang으로 변환해라
vertical$Home.Language[vertical$Home.Language == ""] <- NA # 아무것도 없는 것은 NA로 바꾼다
...
table(x != "Grant.Application.ID")
y<-c("Grant.Application.ID", paste(pre, other, sep = ""))
head(y)
...
head(people)
tbl <- table(vertical$Grant.Application.ID)
peopel <- data.frame(tbl)
Grant.Application.ID1 <- names(tbl)
people1 <- data.frame(numPeople = tbl)

head(people1)
head(people)
table(people$Grant.Application.ID)
...
Sys.setlocale("LC_ALL", "English") # system locale을 english로 변환(날짜변수때문)
Sys.getlocale()
...
save(training, testing, file="grantData.Rdata")
save.image(file="grantData_all.RData")
load("grantData_all.Rdata")
